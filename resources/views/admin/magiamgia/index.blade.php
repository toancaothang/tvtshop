...
<section class="recent" style="margin-top: 0px;">     
        <h4 class="dash-title">Quản lý  mã giảm giá</h4>
        <a style="margin-bottom: 0px;font-size:15px;margin-left:0px" class="button type2" href="{{route('formthemMGG')}}">Thêm mới</a>
        <hr>
        <section class="recent" style="margin-top: 3px">
          <form method="GET" id="header-search" action="{{route('xylytkMGG')}}">
                    ...
                    <div class="activity-card" style="width:1050px">
                      <div class="table-responsive">
                        <table class="table-sortable" id="example" style="margin-bottom: 20px">
                            ...
                            <tbody>
                              @foreach ($dsmagiamgia as $a)
                                <tr>
                                  <td>{{$a->coupon_name}}</td>
                                  <td>{{$a->coupon_code}}</td>
                                  <td>{{$a->coupon_time}}</td>
                                  <td>{{$a->coupon_condition}}</td>
                                  <td>{{$a->coupon_number}}</td>
                                  <td style="width:170px">
                                    <a href="{{route('SuaMGG',['MGG'=>$a->coupon_id])}}" class="button type2">Sửa</a>
                                    <a href="{{route('xylyxoaMGG',['MGG'=>$a->coupon_id])}}" class="button type2">Xoá</a>
                                  </td>
                                </tr>
                              @endforeach  
                            </tbody>
