...
<h3 class="dash-title">Quản lý nhà sản xuất</h3>
<a style="margin-bottom: 0px;font-size:15px;margin-left:0px" class="button type2" href="{{route('formthemNSX')}}">Thêm mới</a>
<hr>
    <section class="recent" style="margin-top: 3px">
      <form method="GET" id="header-search" action="{{route('xylytkNSX')}}">
        <input style=" width:300px;display: inline-block; vertical-align: middle" type="text" name="name" id="name" class="form-control" placeholder="Nhập tên nhà sản xuất" />
                    ...
                    <tbody>
                      @foreach ($dsnhasanxuat as $a)
                        <tr>
                          ...
                          <td>{{$a->info}}</td>
                          <td>
                            <a href="{{route('SuaNSX',['NSX'=>$a->id])}}" class="button type2">Sửa</a>
                            <a href="{{route('xylyxoaNSX',['NSX'=>$a->id])}}" class="button type2">Xoá</a>
                          </td>
                        </tr>
                      @endforeach  
                    </tbody>
